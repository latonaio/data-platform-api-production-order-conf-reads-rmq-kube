...
import (
	dpfm_api_caller "data-platform-api-production-order-conf-reads-rmq-kube/DPFM_API_Caller"
	dpfm_api_input_reader "data-platform-api-production-order-conf-reads-rmq-kube/DPFM_API_Input_Reader"
	dpfm_api_output_formatter "data-platform-api-production-order-conf-reads-rmq-kube/DPFM_API_Output_Formatter"
	"data-platform-api-production-order-conf-reads-rmq-kube/config"
	"encoding/json"
	"fmt"
	"time"
...
func callProcess(rmq *rabbitmq.RabbitmqClient, caller *dpfm_api_caller.DPFMAPICaller, conf *config.Conf, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	res, errs := caller.AsyncProductionOrderConfirmationReads(accepter, &input, &output, l)
	if len(errs) != 0 {
...
func getAccepter(input *dpfm_api_input_reader.SDC) []string {
	...
	if accepter[0] == "All" {
		accepter = []string{
			"Header", "Item", "ItemComponent", "ItemComponentCosting", "ItemComponentDeliveryScheduleLine",
